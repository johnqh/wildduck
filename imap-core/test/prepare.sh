...
echo "Creating user"
USERRESPONSE=`curl --silent -XPOST "http://127.0.0.1:8080/users?accessToken=testtoken123" \
...
MAILBOXLIST=`curl --silent "http://127.0.0.1:8080/users/$USERID/mailboxes?accessToken=testtoken123"`
echo "ML: $MAILBOXLIST"
echo "$MAILBOXLIST" | jq
...
curl --silent -XPOST "http://127.0.0.1:8080/users/$USERID/mailboxes/$INBOXID/messages?date=14-Sep-2013%2021%3A22%3A28%20-0300&unseen=true&accessToken=testtoken123" \
	-H 'Content-type: message/rfc822' \
	--data-binary "@fixtures/fix1.eml"

curl --silent -XPOST "http://127.0.0.1:8080/users/$USERID/mailboxes/$INBOXID/messages?unseen=false&accessToken=testtoken123" \
	-H 'Content-type: message/rfc822' \
...
curl --silent -XPOST "http://127.0.0.1:8080/users/$USERID/mailboxes/$INBOXID/messages?unseen=true&accessToken=testtoken123" \
	-H 'Content-type: message/rfc822' \
