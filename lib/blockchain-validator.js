...
const bs58 = (() => {
    try {
        return require('bs58').default || require('bs58');
    } catch (err) {
        return require('bs58');
    }
})();
...
function isValidBlockchainIdentifier(username) {
    ...
    // 2. Check if it's a base64-encoded EVM address
    if (isBase64EVMAddress(cleanUsername)) {
        return true;
    }

    // 3. Check if it's a Solana address
    if (isSolanaAddress(cleanUsername)) {
        return true;
    }

    // 4. Check if it's an ENS name (.eth or .box) - use original case
    if (isENSName(cleanUsername)) {
        return true;
    }

    // 5. Check if it's an SNS name (.sol) - use original case
...
/**
 * Validates if a string is a base64-encoded EVM address
 * @param {String} encoded - The base64-encoded string to validate
 * @returns {Boolean} - True if valid base64-encoded EVM address
 */
function isBase64EVMAddress(encoded) {
    try {
        // Check if it looks like base64 (proper length and characters)
        ...
        // Check if decoded result is a valid EVM address (40 hex chars)
        if (decoded.length !== 40) {
            return false;
        }

        // Add 0x prefix and validate as EVM address
