...
async function verifySignature(username, signature, message, signerAddress = null) {
    ...
    if (isEVMAddress(cleanUsername) || isBase64EVMAddress(cleanUsername)) {
        return verifyEVMSignature(cleanUsername, signature, message);
        ...
        return verifyEVMSignature(signerAddress, signature, message);
 ...
 */
function verifyEVMSignature(addressOrBase64, signature, message) {
    try {
        let address;
