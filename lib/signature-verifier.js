...
async function verifySignature(username, signature, message) {
    ...
    const indexerUrl = config.mailBoxIndexerUrl || env.get('MAIL_BOX_INDEXER_URL') || env.get('INDEXER_BASE_URL');
    ...
    try {
        ...
        const response = await axios.post(`${indexerUrl}/api/signature/verify`, {
