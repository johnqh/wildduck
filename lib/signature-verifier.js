...
async function verifySignature(username, signature, message) {
    ...
    const indexerUrl = config.mailBoxIndexerUrl || env.get('INDEXER_BASE_URL');
    console.log(`INDEXER_BASE_URL set to ${indexerUrl}`);

    // Bypass signature verification if no indexer URL is configured
    if (!indexerUrl) {
        console.log(`Signature verification bypassed for ${cleanUsername} (INDEXER_BASE_URL not set)`);
        return true; // Always return success when indexer is not configured
    }
    ...
    try {
        ...
        if (isENSName(cleanUsername)) {
            ...
            if (!ownerAddress) {
                throw new Error(
                    `ENS name resolution failed: Could not resolve ${cleanUsername} to an owner address. The ENS name may not exist or may not have an owner set.`
                );
            }
            ...
            if (!ownerAddress) {
                throw new Error(
                    `SNS name resolution failed: Could not resolve ${cleanUsername} to an owner address. The SNS name may not exist or may not have an owner set.`
                );
            }
            addressToVerify = ownerAddress;
        }

        // Call the mail_box_indexer's /api/signature/verify endpoint
        const response = await axios.post(
            `${indexerUrl}/api/signature/verify`,
            {
...
function isENSName(name) {
    ...
    const withoutTLD = lowerName.endsWith('.eth') ? lowerName.slice(0, -4) : lowerName.slice(0, -4);
