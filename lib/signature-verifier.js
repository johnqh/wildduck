...
const { verifyMessage, isAddress: viemIsAddress, recoverMessageAddress } = require('viem');
 ...
 * @param {String} message - The message that was signed (optional - if provided, verifies specific message)
 * @returns {Promise<Boolean>} - True if signature is valid
 */
async function verifySignature(username, signature, message = null) {
    ...
    if (isEVMAddress(cleanUsername) || isBase64EVMAddress(cleanUsername)) {
        ...
        return await verifyEVMSignature(ownerAddress, signature, message);
        ...
        return verifySolanaSignature(ownerAddress, signature, message);
    }
 ...
 * @param {String} message - The original message (optional - if not provided, recovers signer from signature)
 * @returns {Boolean} - True if signature is valid
 */
async function verifyEVMSignature(addressOrBase64, signature, message = null) {
    try {
        let address;
        let signatureHex;
        ...
        if (message) {
            // Use viem to verify the signature against specific message
            ...
            // No specific message required - accept any valid signature from this address
            // Try to recover the signer address from the signature with common message formats
            const commonMessages = [
                '', // Empty message
                'Authentication request',
                'Sign in',
                'Login',
                address, // Sometimes people sign their own address
                `Authenticate ${address}`
            ];
            
            for (const testMessage of commonMessages) {
                try {
                    ...
                    // Continue to next message format
                    continue;
                }
            }
            
            // If none of the common messages work, the signature is not valid for this address
            return false;
        }
    } catch (err) {
