...
class POP3Connection extends EventEmitter {
    ...
    command_PASS(args, next) {
        ...
        let signatureParam = args;
        this.session.user = false;

        // For blockchain auth via POP3 PASS command, args should contain
        ...
        try {
            ...
            // Fall back to treating args as a blockchain signature
        ...
        this._server.onAuth(
            ...
            (err, response) => {
                ...
                if (!response || !response.user) {
                    ...
                    this.send('-ERR [AUTH] ' + ((response && response.message) || 'Authentication failed'));
                    return next();
                }
    ...
    authPlain(plain, next) {
        ...
        // Support both legacy format (3 fields) and blockchain format (5 fields)
        // Standard RFC format: [authzid, username, signature]
        // Blockchain: [authzid, username, signature, message, signerAddress]
        ...
        if (isBlockchainAuth) {
            signature = credentials[2] || '';
            message = credentials[3] || '';
            signerAddress = credentials[4] || undefined;
        } else {
            // Standard format - third field contains signature
            signature = credentials[2] || '';
            message = ''; // Empty message for compatibility
            signerAddress = undefined;
        }
