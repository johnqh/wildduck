...
/**
 * MessageHandler manages email message storage, processing, and retrieval.
 * Handles message indexing, attachment deduplication, encryption, and IMAP operations.
 * 
 * @class MessageHandler
 */
class MessageHandler {
    /**
     * Initialize MessageHandler with database connections and storage backends
     * 
     * @param {Object} options - Configuration options
     * @param {Object} options.database - Main database connection
     * @param {Object} options.redis - Redis connection for caching and notifications
     * @param {Object} [options.users] - Users database connection
     * @param {Object} [options.gridfs] - GridFS connection for attachments
     * @param {Object} [options.attachmentStorage] - Custom attachment storage
     * @param {Object} [options.attachments] - Attachment configuration
    ...
    // Helper method to extract and validate initial data for addAsync
    async _extractInitialData(options) {
        ...
        // get target mailbox data and user data
        ...
        return { mailboxData, userData };
    }

    // Helper method to handle encryption logic
    async _handleEncryption(options, userData, mailboxData) {
        ...
        if (prepared) {
            ...
            if (options.rawchunks && !options.raw) {
                options.raw = options.chunklen ? Buffer.concat(options.rawchunks, options.chunklen) : Buffer.concat(options.rawchunks);
            }

            const rawString = options.raw.toString('binary');
        ...
        if (!alreadyEncrypted) {
            if ((userData.encryptMessages && options.session && options.session.user && !userData.pubKey) || options.targetMailboxEncrypted) {
                if (options.rawchunks && !options.raw) {
                    options.raw = options.chunklen ? Buffer.concat(options.rawchunks, options.chunklen) : Buffer.concat(options.rawchunks);
                }
                
                if (mailboxData.path !== '\\Drafts') {
                    const encrypted = await this.encryptMessageAsync(userData.pubKey, options.raw);
                    options.raw = encrypted.message;
                    options.maildata = this.indexer.getMaildata(prepared.mimeTree);
                }
                
                delete options.prepared;
                prepared = await this.prepareMessageAsync(options);
            } else {
                prepared = await this.prepareMessageAsync(options);
            }
        } else {
    ...
    // Helper method to create cleanup function and store attachments
    async _storeAttachments(mimeTree, maildata) {
        let cleanup = async (err) => {
            if (err) {
    ...
    // Main method for inserting new messages to a mailbox
    // REFACTORED: Helper methods added above to break down complexity
    // - _extractInitialData(), _handleEncryption(), _storeAttachments()
    // TODO: Integrate helper methods into main flow for complete refactoring
