...
const { log: customLog } = require('./logger');
    let main = () => {
        ...
        db.connect(() => {
            let run = async () => {
                ...
                for (let query of queries) {
                    customLog('PARSED QUERY');
                    customLog(util.inspect({ query, parsed: parseSearchQuery(query) }, false, 22, true));
                    customLog('MongoDB');
                    customLog(util.inspect({ query, filter: await getMongoDBQuery(db, new ObjectId('64099fff101ca2ef6aad8be7'), query) }, false, 22, true));
                    customLog('ElasticSearch');
                    customLog(
