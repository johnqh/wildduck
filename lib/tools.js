    ...
    if (validationResult.error && validationResult.error.details) {
        validationResult.error.details.forEach(detail => {
            if (!errors[detail.path]) {
                ...
                // Include the actual value that caused the error for better debugging
                if (detail.context && 'value' in detail.context) {
                    const value = typeof detail.context.value === 'object' ? 
                        JSON.stringify(detail.context.value) : 
