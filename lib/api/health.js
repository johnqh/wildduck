    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                ...
                if (!pingResult.ok) {
                    logError(new Error('MongoDB ping failed'), {
                        userId: req.user,
                        role: req.role,
                        operation: 'health-check'
                    }, 'MongoDB ping returned not ok');
                    res.status(500);
                    return res.json({
                        success: false,
                        message: 'DB is down'
            ...
            logDB('insertOne', 'health', {
                userId: req.user,
                role: req.role
            }, 'Testing MongoDB write capability');
            ...
            logAPI('GET', '/health', {
                ...
                ip: req.ip,
                status: 'success'
            }, 'Health check request completed successfully');
