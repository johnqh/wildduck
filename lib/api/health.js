...
module.exports = (db, server, loggelf) => {
    server.get(
                ...
                response: {
                    200: {
                        ...
                        model: Joi.object({ 
                            success: successRes,
                            api: Joi.object({
                                version: Joi.string().description('WildDuck API version'),
                                name: Joi.string().description('API package name')
                            }).description('Backend API version information'),
                            frontend: Joi.object({
                                version: Joi.string().description('Frontend version'),
                                name: Joi.string().description('Frontend name'),
                                build: Joi.string().optional().description('Frontend build identifier')
                            }).description('Frontend version information')
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Load frontend version information
            let frontendVersion = { version: 'unknown', name: 'Frontend' };
            try {
                const frontendVersionPath = path.join(__dirname, '../../public/version.json');
                if (fs.existsSync(frontendVersionPath)) {
                    frontendVersion = JSON.parse(fs.readFileSync(frontendVersionPath, 'utf8'));
                }
            } catch (err) {
                // Use default values if frontend version can't be loaded
            }
