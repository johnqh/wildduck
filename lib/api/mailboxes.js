...
module.exports = (db, server, mailboxHandler) => {
    ...
    /**
     * Get translated mailbox name based on user language
     * @param {String} originalName - Original mailbox name
     * @param {String} specialUse - Special use flag (e.g., '\\Drafts')
     * @param {String} language - User language code (default: 'en')
     * @returns {String} - Translated name or original name if no translation available
     */
    const getTranslatedMailboxName = (originalName, specialUse, language = 'en') => {
        // Use specialUse for special folders, fallback to originalName for custom folders
        const translationKey = specialUse || originalName;
        
        // Get translations for the user's language, fallback to English
        const userTranslations = translations[language] || translations.en;
        
        // Return translated name if available, otherwise return original name
        return userTranslations[translationKey] || originalName;
    };
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            for (let mailboxData of mailboxes) {
                ...
                let originalName = path.pop();
                
                // Get translated name based on user language
                let name = getTranslatedMailboxName(originalName, mailboxData.specialUse, userData.language);
