...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const putMessageHandler = async (req, res) => {
            ...
            if (typeof result.value.metaData === 'object') {
                ...
                try {
                    ...
                    if (!value || typeof value !== 'object') {
                        throw new Error(`Invalid metaData: expected object, received ${typeof value} (${JSON.stringify(value)})`);
                    }
                } catch (err) {
                    res.status(400);
                    return res.json({
                        error: err.message || 'metaData value must be valid JSON object string',
                        code: 'InputValidationError'
                    });
                }
            }
        ...
        if (moveTo) {
            ...
            try {
                ...
                if (!lock.success) {
                    throw new Error(`Failed to acquire write lock for mailbox operation (timeout: ${1 * 60 * 1000}ms)`);
                }
                log.verbose(
                    'API',
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // EXTENSIVE LOGGING: Request start
            console.log('=== POST /users/:user/mailboxes/:mailbox/messages START ===');
            ...
            // EXTENSIVE LOGGING: Validation start
            console.log('Validation schema keys:', Object.keys(schema.describe().keys));
            console.log('req.params before validation:', JSON.stringify(req.params, null, 2));
            ...
            console.log('Validation result error:', result.error ? result.error.message : 'none');
            console.log('Validation result value keys:', result.value ? Object.keys(result.value) : 'none');
            ...
            // EXTENSIVE LOGGING: Permissions check
            console.log('Permission check - req.user:', req.user);
            console.log('Permission check - result.value.user:', result.value.user);
            console.log('Permission check - req.role:', req.role);

            // permissions check
            if (req.user && req.user === result.value.user) {
                console.log('Using createOwn permissions for messages');
                req.validate(roles.can(req.role).createOwn('messages'));
            } else {
                console.log('Using createAny permissions for messages');
            ...
            // EXTENSIVE LOGGING: Database operations start
            console.log('Looking up mailbox - mailbox ID:', mailbox);
            console.log('Looking up mailbox - user ID:', user);
            ...
            try {
                ...
                console.log('Mailbox lookup result:', mailboxData ? 'found' : 'not found');
                if (mailboxData) {
                    console.log('Mailbox data keys:', Object.keys(mailboxData));
                    console.log('Mailbox path:', mailboxData.path);
                }
            } catch (err) {
                console.log('MAILBOX LOOKUP ERROR:', err.message);
                res.status(500);
                return res.json({
                    error: 'MongoDB Error: ' + err.message,
                    code: 'InternalDatabaseError'
            ...
            console.log('Looking up user - user ID:', user);
            ...
            try {
                ...
                if (userData) {
                    ...
                    console.log('User disabled:', userData.disabled);
                    console.log('User suspended:', userData.suspended);
                    console.log('User quota:', userData.quota);
                    console.log('User storageUsed:', userData.storageUsed);
                }
            } catch (err) {
            ...
            // EXTENSIVE LOGGING: Message creation start
            console.log('About to create message with messageHandler.addAsync');
            console.log('Message data summary:');
            console.log('- from:', data.from);
            console.log('- to:', data.to ? data.to.length + ' recipients' : 'none');
            console.log('- subject:', data.subject);
            console.log('- has text:', !!data.text);
            console.log('- has html:', !!data.html);
            console.log('- raw message length:', raw ? raw.length : 'none');
            ...
            try {
                ...
                console.log('Message data:', messageData ? Object.keys(messageData) : 'none');
            } catch (err) {
                console.log('MESSAGE CREATION ERROR:', err.message);
                console.log('Error stack:', err.stack);
                console.log('Error code:', err.code);
                console.log('Error responseCode:', err.responseCode);
                console.log('Error imapResponse:', err.imapResponse);
                res.status(err.responseCode || 500);
                return res.json({
                    error: err.message,
            ...
            console.log('=== POST /users/:user/mailboxes/:mailbox/messages END ===');
            return res.json(response);
        })
    );
