...
const { logAPI, logError, logPerformance } = require('../logger');
...
const IndexerHelper = require('../indexer-helper');
...
module.exports = (db, server, messageHandler, userHandler, settingsHandler) => {
                                    ...
                                    let addToDeliveryQueue = next => {
                                        ...
                                        messageHandler.counters.ttlcounter('wdr:' + userData._id.toString(), 0, maxRecipients, false, (err, result) => {
                                            ...
                                            if (!success || sent + compiledEnvelope.length > maxRecipients) {
                                                ...
                                                let err = new Error(
                                                    'You reached a daily sending limit for your account' +
                                                        (ttl && ttl > 0 ? '. Limit expires in ' + ttlHuman : '')
                                                );
                                                err.responseCode = 403;
                                        ...
                                        messageHandler.encryptMessage(
                                            ...
                                            (err, encrypted) => {
                                                ...
                                                messageHandler.add(messageOptions, (err, success, info) => {
                                                    ...
                                                    let done = () => {
                                                        // Award reward points for email action (async, non-blocking)
                                                        if (IndexerHelper.isConfigured() && userData.username && !options.uploadOnly && !options.isDraft) {
                                                            // Determine action type
                                                            let rewardAction = 'send'; // Default is send
                                                            if (options.reference && options.reference.action) {
                                                                if (options.reference.action === 'forward') {
                                                                    rewardAction = 'forward';
                                                                } else if (options.reference.action === 'reply' || options.reference.action === 'replyAll') {
                                                                    rewardAction = 'reply';
                                                                }
                                                            }

                                                            // Call IndexerHelper.addRewardPoints asynchronously (don't wait for result)
                                                            IndexerHelper.addRewardPoints(userData.username, rewardAction).catch(err => {
                                                                log.error('API', 'REWARDFAIL user=%s action=%s error=%s', userData.username, rewardAction, err.message);
                                                            });
                                                        }
    ...
    // Shared submit handler logic
    const handleSubmitMessage = async (req, res, targetUserId) => {
        const startTime = Date.now();
        res.charSet('utf-8');

        // Update req.params.user with the target user ID
        req.params.user = targetUserId;

        const { requestBody, queryParams } = req.route.spec.validationObjs;
        ...
        // Check indexer verification if provided - this replaces role-based permissions
        const indexerAddress = config.api?.indexerAddress;
        let indexerVerified = false;

        if (result.value.indexer && indexerAddress && IndexerHelper.isConfigured()) {
            logAPI(
                'POST',
                req.route.path,
                {
                    userId: req.user,
                    role: req.role,
                    targetUser: result.value.user.toString()
                },
                'Verifying indexer signature'
            );

            try {
                const isValid = await IndexerHelper.verify(indexerAddress, result.value.indexer.message, result.value.indexer.signature);
                ...
                if (!isValid) {
                    logAPI(
                        ...
                        req.route.path,
                        {
                            ...
                            role: req.role,
                            targetUser: result.value.user.toString(),
                            error: 'IndexerVerificationFailed'
                        },
                        'Indexer verification failed'
                    );
                    res.status(400);
                    return res.json({
                        error: 'Invalid indexer signature',
                        code: 'IndexerVerificationFailed'
                    });
                }
                ...
                logAPI(
                    ...
                    'Indexer verification successful - bypassing role-based permissions'
                );
                ...
                logError(
                    ...
                    'Failed to verify with indexer'
                );
                res.status(500);
                return res.json({
                    error: 'Failed to verify with indexer service',
                    code: 'IndexerServiceError'
                });
            }
        }

        // Only check permissions if indexer verification was not successful
        if (!indexerVerified) {
            // Standard permissions check
            if (req.user && req.user === result.value.user.toString()) {
        ...
        logAPI(
            ...
            'Submitting message for delivery'
        );
        ...
        logPerformance(
            ...
            'submitMessage request completed'
        );
        ...
        logAPI(
            ...
            'submitMessage request completed successfully'
        );
    ...
    server.post(
        {
            ...
            validationObjs: {
                requestBody: {
                    ...
                    indexer: Joi.object()
                        .keys({
                            message: Joi.string().required().description('Message to verify'),
                            signature: Joi.string().required().description('Signature of the message')
                        })
                        .description('Optional indexer verification data')
                },
                queryParams: {},
                pathParams: {
                    user: userId
        ...
        tools.responseWrapper(async (req, res) => handleSubmitMessage(req, res, req.params.user))
    );

    // New endpoint to submit message by username - same params as /users/:user/submit but with username
    server.post(
        {
            name: 'submitMessageByUsername',
            path: '/users/name/:username/submit',
            tags: ['Submission'],
            summary: 'Submit a Message for Delivery by Username',
            description: 'Use this method to send emails from a user account identified by username',
            validationObjs: {
                requestBody: {
                    mailbox: Joi.string().hex().lowercase().length(24).description('ID of the Mailbox'),
                    from: AddressOptionalName.description('Address for the From: header'),
                ...
                queryParams: {},
                pathParams: {
                    username: Joi.string().empty('').lowercase().min(1).max(255).required().description('Username of the User')
                },
                response: {
                    200: {
                        description: 'Success',
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let userData;
            try {
                userData = await userHandler.get(req.params.username, { fields: { _id: true } });
                if (!userData) {
                    res.status(404);
                    return res.json({
            ...
            // Step 2: Call the shared handler with the user ID
            return handleSubmitMessage(req, res, userData._id.toString());
        })
    );
};
