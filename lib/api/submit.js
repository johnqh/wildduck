...
// Function to verify signature with indexer service
async function verifySignatureWithIndexer(message, signature) {
    return new Promise((resolve, reject) => {
        const indexerWalletAddress = env.get('INDEXER_WALLET_ADDRESS');
        const indexerUrl = env.get('INDEX_IP_ADDRESS') || 'localhost';
        
        if (!indexerWalletAddress) {
            return reject(new Error('INDEXER_WALLET_ADDRESS environment variable not set'));
        }

        const postData = JSON.stringify({
            walletAddress: indexerWalletAddress,
            message,
            signature
        });

        const isHttps = indexerUrl.startsWith('https://');
        const client = isHttps ? https : http;
        const url = new URL(`${isHttps ? 'https' : 'http'}://${indexerUrl.replace(/^https?:\/\//, '')}:42069/api/signature/verify`);
        ...
        const req = client.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    if (res.statusCode === 200 && response.isValid) {
                        resolve(response);
                    } else {
                        reject(new Error(response.error || 'Signature verification failed'));
                    }
                } catch (err) {
                    reject(new Error('Invalid response from indexer service'));
                }
            });
        });

        req.on('error', (err) => {
            reject(new Error(`Failed to connect to indexer service: ${err.message}`));
        });
...
module.exports = (db, server, messageHandler, userHandler, settingsHandler) => {
    ...
    server.post(
            ...
            validationObjs: {
                requestBody: {
                    ...
                    authentication: Joi.object()
                        .keys({
                            message: Joi.string().required().description('Message that was signed'),
                            signature: Joi.string().required().description('Cryptographic signature')
                        })
                        .required()
                        .description('Authentication object for signature verification'),
                    to: Joi.array()
                        .items(
                            Joi.object({
                                name: Joi.string().empty('').max(255).description('Name of the sender'),
                    ...
                    200: {
                        ...
                        model: Joi.object({
                            ...
                            message: Joi.object({
                                ...
                                .description('Information about submitted Message')
                                .$_setFlag('objectName', 'MessageWithQueueId')
                        }).$_setFlag('objectName', 'SubmitEmailResponse')
                    }
                }
            }
        },
        tools.responseWrapper(async (req, res) => {
            ...
            // Validate caller IP address matches indexer IP
            const indexerIp = env.get('INDEX_IP_ADDRESS');
            const clientIp = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || 
                           (req.connection.socket ? req.connection.socket.remoteAddress : null);
            
            if (!indexerIp) {
                log.error('API', 'INDEXER_IP_NOT_CONFIGURED');
                res.status(500);
                return res.json({
                    error: 'Server configuration error: Indexer IP not configured',
                    code: 'ServerConfigurationError'
                });
            }

            // Normalize IP addresses for comparison (handle IPv6 mapped IPv4)
            ...
            const normalizedIndexerIp = normalizeIp(indexerIp);

            if (normalizedClientIp !== normalizedIndexerIp && normalizedClientIp !== '127.0.0.1' && normalizedClientIp !== 'localhost') {
                log.error('API', 'UNAUTHORIZED_IP_ACCESS client=%s expected=%s', normalizedClientIp, normalizedIndexerIp);
                res.status(403);
                return res.json({
                    error: 'Access denied: Request must come from authorized indexer IP address',
                    code: 'UnauthorizedIpAddress'
                });
            }

            // Verify signature with indexer service
            try {
                await verifySignatureWithIndexer(result.value.authentication.message, result.value.authentication.signature);
                log.info('API', 'SIGNATURE_VERIFIED successfully from IP=%s', normalizedClientIp);
            } catch (verifyError) {
                log.error('API', 'SIGNATURE_VERIFICATION_FAILED error=%s', verifyError.message);
                res.status(401);
                return res.json({
            ...
            // Resolve user from 'from' address
            let user;
            if (result.value.from && result.value.from.address) {
                try {
                    const userData = await userHandler.asyncGet(result.value.from.address, false);
                    if (!userData) {
                        res.status(404);
                        return res.json({
                            error: `User not found for address: ${result.value.from.address}`,
                            code: 'UserNotFound'
                        });
                    }
                    user = userData._id;
                } catch (err) {
                    log.error('API', 'USER_LOOKUP_FAILED address=%s error=%s', result.value.from.address, err.message);
                    res.status(500);
                    return res.json({
                        error: 'Failed to lookup user for from address',
                        code: 'UserLookupError'
                    });
                }
            } else {
                res.status(400);
