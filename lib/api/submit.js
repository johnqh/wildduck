...
async function verifySignatureWithIndexer(message, signature) {
    return new Promise((resolve, reject) => {
        ...
        // Bypass verification if INDEXER_BASE_URL is not set
        if (!indexerUrl) {
            return resolve({ isValid: true, bypassed: true });
        }
...
module.exports = (db, server, messageHandler, userHandler, settingsHandler) => {
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Check if indexer service is configured (bypass validation if not configured)
            const clientIp =
                req.ip || req.connection.remoteAddress || req.socket.remoteAddress || (req.connection.socket ? req.connection.socket.remoteAddress : null);
            ...
            try {
                ...
                log.info('API', 'SIGNATURE_VERIFIED successfully from IP=%s', clientIp);
            } catch (verifyError) {
