...
const consts = require('../consts'); // eslint-disable-line no-unused-vars
...
const isemail = require('isemail'); // eslint-disable-line no-unused-vars
const {
    publish,
    ADDRESS_USER_CREATED, // eslint-disable-line no-unused-vars
    ADDRESS_USER_DELETED, // eslint-disable-line no-unused-vars
...
module.exports = (db, server, userHandler, settingsHandler) => {
    // Helper function to return 400 for disabled endpoints
    const createDisabledEndpointHandler = (operation) =>
        tools.responseWrapper(async (req, res) => {
            res.status(400);
            return res.json({
                error: `${operation} has been disabled. This endpoint is no longer available.`,
    ...
    // DISABLED: POST /users/:user/addresses endpoint - Address creation disabled
    /*
    server.post(
        {
                ...
                if (address.charAt(0) === '*') {
                    ...
                    try {
                        ...
                        if (partial.indexOf('*') >= 0) {
                            throw new Error(`Invalid wildcard address "${address}": only one wildcard (*) is allowed per address`);
                        }
                        ...
                        if (partial.substr(0, partial.indexOf('@')).length > consts.MAX_ALLOWED_WILDCARD_LENGTH) {
                            throw new Error(`Invalid wildcard address "${address}": username part too long (max ${consts.MAX_ALLOWED_WILDCARD_LENGTH} characters)`);
                        }

                        // result neewds to be a valid email
                        if (!isemail.validate(partial)) {
                            throw new Error(`Invalid wildcard address "${address}": does not result in a valid email format`);
                        }
                    } catch (err) {
                        res.status(400);
                        return res.json({
                            error: err.message || 'Invalid wildcard address, use "*@domain" or "user@*"',
                            code: 'InputValidationError'
                        });
                    }
                }
    ...
    */
    ...
    // Replacement endpoint that returns 400 error
    server.post(
        {
            path: '/users/:user/addresses',
            summary: 'Create new Address (DISABLED)',
            name: 'createUserAddress',
            tags: ['Addresses'],
            validationObjs: {
                requestBody: {},
        ...
        createDisabledEndpointHandler('Address creation')
    );
    ...
    // DISABLED: PUT /users/:user/addresses/:id endpoint - Address updates disabled
    /*
    server.put(
        {
    ...
    */
    ...
    server.put(
        {
            path: '/users/:user/addresses/:id',
            summary: 'Update Address information (DISABLED)',
            name: 'updateUserAddress',
            tags: ['Addresses'],
            validationObjs: {
                requestBody: {},
        ...
        createDisabledEndpointHandler('Address updates')
    );

    // DISABLED: DELETE /users/:user/addresses/:address endpoint - Address deletion disabled
    /*
    server.del(
        {
            path: '/users/:user/addresses/:address',
    ...
    server.del(
        {
            path: '/users/:user/addresses/:address',
            name: 'deleteUserAddress',
            summary: 'Delete an Address (DISABLED)',
            tags: ['Addresses'],
            validationObjs: {
                requestBody: {},
                pathParams: {
        ...
        createDisabledEndpointHandler('Address deletion')
    );
