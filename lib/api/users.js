...
module.exports = (db, server, userHandler, settingsHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                ...
                res.status(500);
                return res.json({
                    error: 'Database query failed while fetching user list: ' + err.message,
                    ...
                    query_options: {
                        limit: opts.limit,
                        sort: opts.sortAscending ? 'ascending' : 'descending',
                        fields: Object.keys(opts.fields.projection || {})
                    }
                });
            }
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Auto-generate name and address if not provided
            if (!values.name && values.username) {
                values.name = values.username;
            }
            if (!values.address && values.username) {
                const emailDomain = config.emailDomain || 'example.com';
                values.address = `${values.username}@${emailDomain}`;
            }
    ...
    server.post(
        {
            ...
            name: 'createExport',
            description:
                'Export data for matching users. Export dump does not include emails, only account structure (user data, mailboxes, filters, etc.). A special "export"-role access token is required for exporting and importing.',
            validationObjs: {
                requestBody: {
