...
module.exports = (db, server, userHandler, settingsHandler) => {
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            logAPI(
                ...
                '/users',
                {
                    ...
                    forward: req.params.forward
                },
                'Started getUsers request'
            );
            ...
            if (result.error) {
                logAPI(
                    ...
                    'Validation failed for getUsers request',
                    {
                        validationErrors: tools.validationErrors(result)
                    }
                );
                res.status(400);
                return res.json({
            ...
            logDB(
                ...
                'Querying users collection for getUsers'
            );
            ...
            try {
                ...
                logDB(
                    ...
                    'users',
                    {
                        userId: req.user,
                        role: req.role,
                        resultsCount: (listingWrapper.listing.results || []).length,
                        total
                    },
            ...
            logAPI(
                ...
                'getUsers request completed successfully'
            );
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Extract username from email format if it contains @
            if (values.username && values.username.includes('@')) {
                values.username = values.username.split('@')[0];
            }
    ...
    server.put(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Check if crypto emails mode is enabled and password is being updated
            if (tools.runningCryptoEmails() && (result.value.password || values.password)) {
                res.status(403);
                return res.json({
                    error: 'Password updates are not available in crypto emails mode',
                    code: 'PasswordUpdateNotAvailable'
                });
            }
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // In crypto mode, password reset is not allowed
            if (tools.runningCryptoEmails()) {
                res.status(400);
                return res.json({
                    error: 'Password reset is not available in crypto emails mode',
                    code: 'PasswordResetNotAvailable'
                });
            }
