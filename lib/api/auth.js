...
module.exports = (db, server, userHandler) => {
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Preauth is not needed with crypto email mode
            if (tools.runningCryptoEmails()) {
                res.status(400);
                return res.json({
                    error: 'Endpoint not available in crypto emails mode',
    ...
    server.post(
            ...
            validationObjs: {
                requestBody: {
                    username: Joi.alternatives()
                        ...
                        .required()
                        .description('Username or E-mail address'),
                    password: tools.runningCryptoEmails()
                        ? Joi.string().max(256).optional().description('Password (ignored in crypto emails mode)')
                        : Joi.string().max(256).required().description('Password'),

                    emailDomain: tools.runningCryptoEmails()
                        ? Joi.string().hostname().optional().description('Email domain (ignored in crypto emails mode)')
                        : Joi.string().hostname().optional().description('Email domain for new user accounts'),

                    // Web3/SIWE authentication fields
                    signature: Joi.string().optional().description('Cryptographic signature for Web3 authentication'),
                    nonce: Joi.string().optional().description('Nonce used in the signed message'),
                    message: Joi.string().optional().description('The message that was signed (e.g., SIWE message)'),
                    signer: Joi.string().optional().description('The address/public key of the signer'),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // In crypto mode, check indexer authentication first (if configured)
            if (cryptoEmails && IndexerHelper.isConfigured()) {
                try {
                    // Call indexer's /authenticate endpoint
                    const indexerAuthSuccess = await IndexerHelper.authenticate({
                        username: result.value.username,
                        message: result.value.message,
                        signature: result.value.signature,
                        signer: result.value.signer
                    });

                    if (!indexerAuthSuccess) {
                        res.status(403);
                        return res.json({
                            error: 'Indexer authentication failed',
                            code: 'IndexerAuthFailed'
                        });
                    }
                } catch (err) {
                    res.status(500);
            ...
            // Extract username from email format if it contains @ (for standard mode)
            ...
            let errorMessage;
            // Standard mode: require password authentication
            try {
                [authData, user] = await userHandler.asyncAuthenticate(username, result.value.password, result.value.scope, meta, !cryptoEmails);
            } catch (err) {
                errorMessage = err.message;
            }

            if (!authData && cryptoEmails) {
                try {
                    // Extract username from email format for crypto mode too
                    let cryptoUsername = result.value.username;
                    if (cryptoUsername && cryptoUsername.includes('@')) {
                        cryptoUsername = cryptoUsername.split('@')[0];
                    }
                    let userCreateData = {
                        username: cryptoUsername,
                        password: result.value.password || 'default',
                        name: cryptoUsername
                    };

                    // Use custom email domain if provided
                    if (result.value.emailDomain) {
                        userCreateData.address = `${cryptoUsername}@${result.value.emailDomain}`;
                    }

                    await userHandler.create(userCreateData);
                    [authData, user] = await userHandler.asyncAuthenticate(cryptoUsername, result.value.password, result.value.scope, meta, !cryptoEmails);
                } catch (err) {
                    errorMessage = err.message;
                }
            }
