...
const { sendErrorResponse } = require('./error-utils');
    ...
    server.post(
        {
            ...
            validationObjs: {
                requestBody: {
                    ...
                    message: Joi.string().required().description('SIWE/SIWS message that was signed'),
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            try {
                ...
                const additionalInfo = {
                    ...
                    signature: result.value.signature?.substring(0, 20) + '...',
                    nonce: result.value.nonce,
                    message: result.value.message,
                    protocol: result.value.protocol,
                    scope: result.value.scope
                };
                if (user) additionalInfo.id = user.toString();
                return sendErrorResponse(res, err, additionalInfo);
            }

            if (!authData) {
                const error = new Error(`Authentication failed for username "${result.value.username}" with signature "${result.value.signature?.substring(0, 20)}..." and nonce "${result.value.nonce}"`);
