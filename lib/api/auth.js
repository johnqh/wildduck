...
module.exports = (db, server, userHandler) => {
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Check if crypto emails mode is enabled - return 400 if it is
    ...
    server.post(
            ...
            validationObjs: {
                requestBody: {
                    ...
                    password: tools.runningCryptoEmails() ?
                        Joi.string().max(256).optional().description('Password (optional in crypto emails mode)') :
                        Joi.string().max(256).required().description('Password'),

                    emailDomain: tools.runningCryptoEmails() ?
                        Joi.string().hostname().required().description('Email domain for new user accounts (required in crypto emails mode)') :
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            let authData, user, userData;

            // Check if crypto emails mode is enabled
            if (tools.runningCryptoEmails()) {
                // Crypto emails mode: skip password, get or create user
                let username = result.value.username;
                let emailDomain = result.value.emailDomain;
                ...
                if (!userData) {
                    try {
                        ...
                        let userId = await userHandler.create(userCreateData);
                        userData = await db.users.collection('users').findOne({ _id: userId });
                    } catch (err) {
                        res.status(500);
                        return res.json({
                            error: err.message,
                            code: err.code || 'UserCreateFailed'
                        });
                    }
                }

                // Build auth response for crypto emails mode
                authData = {
                    user: userData._id,
                    username: userData.username,
                    address: userData.addresses && userData.addresses[0] ? userData.addresses[0].address : emailAddress,
                    scope: result.value.scope,
                    require2fa: userData.enabled2fa || [],
                    requirePasswordChange: userData.passwordChange || false
                };
            } else {
