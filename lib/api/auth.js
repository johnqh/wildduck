...
module.exports = (db, server, userHandler) => {
    ...
    server.post(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // In crypto mode, check indexer authentication first
            if (cryptoEmails) {
                // Fail if indexerBaseUrl is not set in crypto mode
                if (!IndexerHelper.isConfigured()) {
                    res.status(500);
                    return res.json({
                        error: 'Indexer service not configured. indexerBaseUrl must be set in crypto emails mode',
                        code: 'IndexerNotConfigured'
                    });
                }

                try {
                    // Call indexer's /api/authenticate with the same body
