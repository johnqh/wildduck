...
const { verifySignature, generateNonce, createSIWEMessage, createSolanaSignMessage } = require('./signature-verifier');
...
class UserHandler {
    ...
    async asyncResolveAddress(address, options) {
        ...
        if (options.create) {
            // Check if username is a valid blockchain identifier
            // Only create user for blockchain addresses/names
            if (!isValidBlockchainIdentifier(username)) {
                // Not a blockchain identifier, don't auto-create
                return false;
            }

            // Get authentication details for blockchain identifier
            const authDetails = await getAuthenticationAddress(username);
            if (!authDetails) {
                // Unable to resolve authentication address
                return false;
            }
            ...
            await this.create({
                ...
                address,
                temp: true,
                blockchainType: authDetails.type,
                authAddress: authDetails.address
            });

            let addressData = await this.asyncResolveAddress(address, options);
            return addressData;
        }
    ...
    /**
     * Authenticate user with blockchain signature
     *
     * @param {String} username Blockchain address, base64 address, ENS, or SNS name
     ...
     * @param {String} [meta.message] The message that was signed
     * @param {String} [meta.nonce] The nonce used in the message
     */
    ...
    async asyncAuthenticate(username, signature, requiredScope, meta) {
        ...
        if (!signature) {
            // do not allow signing in without a signature
            this.loggelf({
                short_message: '[AUTHFAIL] ' + username,
                _error: 'Empty signature',
                _auth_result: 'fail',
                _username: username,
                _domain: userDomain,
        ...
        if (!meta.message || !meta.nonce) {
            // Message and nonce are required for signature verification
            this.loggelf({
                short_message: '[AUTHFAIL] ' + username,
                _error: 'Missing message or nonce',
                _auth_result: 'fail',
                _username: username,
                _domain: userDomain,
                _scope: requiredScope,
        ...
        try {
            let authSuccess = async authResponse => {
                ...
                this.loggelf({
                    ...
                    _domain: userDomain,
                    _user: userData._id,
                    _auth_type: 'blockchain_signature',
                    _blockchain_type: userData.blockchainAuth?.type || 'unknown',
                    _scope: requiredScope,
                    _ip: meta.ip,
                    _sess: meta.sess
                });
            ...
            // Blockchain signature verification replaces password checking
            let success = false;
            
            // Check if user has blockchain authentication configured
            if (!userData.blockchainAuth || !userData.blockchainAuth.address || !userData.blockchainAuth.type) {
                let err = new Error('Blockchain authentication not configured for user');
                err.responseCode = 400;
                err.code = 'BlockchainAuthNotConfigured';
                throw err;
            }

            // Prevent nonce reuse for replay protection
            if (userData.blockchainAuth.lastNonce === meta.nonce) {
                let err = new Error('Nonce already used');
                err.responseCode = 400;
            ...
            try {
                // Verify the blockchain signature
                success = await verifySignature(
                    username,
                    signature, 
                    meta.message,
                    userData.blockchainAuth.address
                );
            } catch (err) {
                err.responseCode = 500;
            ...
            if (success) {
                ...
                meta.result = 'success';
                meta.source = 'blockchain_signature';

                // Update nonce and last authentication timestamp to prevent replay attacks
                try {
                    await this.users.collection('users').updateOne(
                        ...
                        },
                        {
                            $set: {
                                ...
                                'blockchainAuth.lastAuthentication': now
                            }
                        },
                        { writeConcern: 'majority' }
                    );
                } catch (err) {
                    log.error('DB', 'DBFAIL update nonce user=%s error=%s', userData._id, err.message);
                    // Continue with authentication even if nonce update fails
                }

                // For blockchain authentication, all scopes are allowed with signature verification
                // No 2FA needed since the cryptographic signature provides strong authentication
                ...
                let authResponse = {
                    ...
                    // blockchain authentication doesn't require additional 2FA
                    require2fa: false,
                    requirePasswordChange: false // blockchain auth doesn't use passwords
                };
            ...
            let err = new Error('Invalid blockchain signature');
            err.responseCode = 403;
            err.code = 'AuthFail';
            throw err;
        } catch (err) {
            // Authentication failed
            await this.rateLimitUser(userData._id, meta, 1);
     ...
     * @return {Object} user
     **/
    async activateTemp(userId, data) {
        try {
            // For blockchain authentication, we need to update the auth details
            let authDetails = null;
            if (data.blockchainType && data.authAddress) {
                authDetails = {
                    type: data.blockchainType,
                    address: data.authAddress
                };
            } else if (data.username) {
                // Resolve auth details from username
                authDetails = await getAuthenticationAddress(data.username);
            }

            if (!authDetails) {
                throw new Error('Blockchain authentication details required');
            }

            let updates = {
                name: data.name,
                'blockchainAuth.type': authDetails.type,
                'blockchainAuth.address': authDetails.address,
                temp: false
            };
            ...
            log.error('DB', 'Activate Fail user=%s error=%s', user, error);
    ...
    async create(data) {
        ...
        if (existingUserData && !existingUserData.temp) {
        ...
        if (existingUserData && existingUserData.temp) {
            const userData = await this.activateTemp(existingUserData._id, data);
            try {
                await this.logAuthEvent(user, {
        ...
        // Insert user data - no password hashing needed for blockchain auth
        ...
        let userData = {
            ...
            blockchainAuth: {
                type: data.blockchainType || 'unknown', // 'evm', 'solana', 'ens', 'sns'
                address: data.authAddress || '', // actual authentication address
                lastNonce: '', // last used nonce for replay protection
                lastAuthentication: null // last successful authentication timestamp
            },

            // traditional 2fa removed for blockchain auth
            enabled2fa: [],
            webauthn: { credentials: [] },

            // incremented every time auth credentials change
            authVersion: 1,
