'use strict';

const config = require('wild-config');
const mongodb = require('mongodb');
const Redis = require('ioredis');
const redisUrl = require('./redis-url');
const log = require('npmlog');
const packageData = require('../package.json');

const MongoClient = mongodb.MongoClient;

module.exports.database = false;
module.exports.gridfs = false;
module.exports.users = false;
module.exports.senderDb = false;

let getDBConnection = async (main, config, callback) => {
    try {
        if (main) {
            if (!config) {
                log.verbose('DB', 'No database config provided, using main connection');
                return callback(null, false);
            }
            if (config && !/[:/]/.test(config)) {
                log.verbose('DB', 'Using database name "%s" from main connection', config);
                return callback(null, main.db(config));
            }
        }
        
        log.info('DB', 'Attempting to connect to MongoDB: %s', config ? config.replace(/\/\/([^:]+):([^@]+)@/, '//***:***@') : 'undefined');
        
        // Use promise-based connection for Mac compatibility with timeout options
        const connectionOptions = {
            serverSelectionTimeoutMS: 10000, // 10 second timeout
            connectTimeoutMS: 10000, // 10 second timeout
            socketTimeoutMS: 0, // Disable socket timeout
            maxPoolSize: 10, // Maximum number of connections
            minPoolSize: 1, // Minimum number of connections
            maxIdleTimeMS: 30000, // Close connections after 30 seconds of inactivity
        };
        
        const client = await MongoClient.connect(config, connectionOptions);
        let db = client;
        
        if (main && client.s && client.s.options && client.s.options.dbName) {
            db = client.db(client.s.options.dbName);
        }
        
        log.info('DB', 'Successfully connected to MongoDB: %s', config ? config.replace(/\/\/([^:]+):([^@]+)@/, '//***:***@') : 'undefined');
        return callback(null, db);
    } catch (err) {
        log.error('DB', 'Failed to connect to MongoDB: %s. Error: %s', config ? config.replace(/\/\/([^:]+):([^@]+)@/, '//***:***@') : 'undefined', err.message);
        return callback(err);
    }
};

module.exports.connect = async callback => {
    try {
        const REDIS_CONF = Object.assign(
            {
                // some defaults
                maxRetriesPerRequest: null,
                showFriendlyErrorStack: true,
                retryStrategy(times) {
                    const delay = !times ? 1000 : Math.min(2 ** times * 500, 15 * 1000);
                    log.info('Redis', 'Connection retry times=%s delay=%s', times, delay);
                    return delay;
                },
                connectionName: `${packageData.name}@${packageData.version}[${process.pid}]`
            },
            typeof config.dbs.redis === 'string' ? redisUrl(config.dbs.redis) : config.dbs.redis || {}
        );

        module.exports.redisConfig = REDIS_CONF;
        module.exports.queueConf = {
            connection: Object.assign({ connectionName: `${REDIS_CONF.connectionName}[notify]` }, REDIS_CONF),
            prefix: `wd:bull`
        };
        
        log.info('Redis', 'Attempting to connect to Redis: %s:%s (db:%s)', 
            REDIS_CONF.host || 'localhost', 
            REDIS_CONF.port || 6379, 
            REDIS_CONF.db || 0
        );
        
        module.exports.redis = new Redis(REDIS_CONF);
        
        // Add Redis connection event handlers
        module.exports.redis.on('connect', () => {
            log.info('Redis', 'Successfully connected to Redis: %s:%s (db:%s)', 
                REDIS_CONF.host || 'localhost', 
                REDIS_CONF.port || 6379, 
                REDIS_CONF.db || 0
            );
        });
        
        module.exports.redis.on('error', (err) => {
            log.error('Redis', 'Redis connection error: %s', err.message);
        });
        
        module.exports.redis.on('close', () => {
            log.warn('Redis', 'Redis connection closed');
        });
        
        module.exports.redis.on('reconnecting', (delay) => {
            log.info('Redis', 'Redis reconnecting in %sms', delay);
        });

        await new Promise((resolve, reject) => {
            getDBConnection(false, config.dbs.mongo, (err, db) => {
                if (err) {
                    return reject(err);
                }

                if (db.s && db.s.options && db.s.options.dbName) {
                    module.exports.database = db.db(db.s.options.dbName);
                } else {
                    module.exports.database = db;
                }

                getDBConnection(db, config.dbs.gridfs, (err, gdb) => {
                    if (err) {
                        return reject(err);
                    }
                    module.exports.gridfs = gdb || module.exports.database;

                    getDBConnection(db, config.dbs.users, (err, udb) => {
                        if (err) {
                            return reject(err);
                        }
                        module.exports.users = udb || module.exports.database;

                        getDBConnection(db, config.dbs.sender, (err, sdb) => {
                            if (err) {
                                return reject(err);
                            }
                            module.exports.senderDb = sdb || module.exports.database;

                            resolve();
                        });
                    });
                });
            });
        });

        return callback();
    } catch (err) {
        return callback(err);
    }
};
