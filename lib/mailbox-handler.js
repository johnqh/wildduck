...
/**
 * MailboxHandler manages IMAP mailbox operations including creation, deletion,
 * renaming, and subscription management. Handles special-use folders and quotas.
 * 
 * @class MailboxHandler
 */
class MailboxHandler {
    /**
     * Initialize MailboxHandler with database connections and notification system
     * 
     * @param {Object} options - Configuration options
     * @param {Object} options.database - Main database connection
     * @param {Object} [options.users] - Users database connection
     * @param {Object} options.redis - Redis connection for notifications
     * @param {Object} [options.notifier] - Custom IMAP notifier instance
    ...
    async createAsync(user, path, opts) {
        ...
        if (mailboxData) {
            const err = new Error(`Mailbox "${path}" already exists`);
        ...
        if (mailboxCountForUser > (await this.settingsHandler.get('const:max:mailboxes'))) {
            const err = new Error(`Cannot create mailbox: maximum limit of ${await this.settingsHandler.get('const:max:mailboxes')} mailboxes reached`);
            err.code = 'CANNOT';
            err.responseCode = 400;
            throw err;
        }
            ...
            (err, mailboxData) => {
                ...
                this.database.collection('mailboxes').findOne(
                    ...
                    (err, existing) => {
                        ...
                        if (existing) {
                            const err = new Error(`Cannot rename mailbox: target "${newname}" already exists`);
                            err.code = 'ALREADYEXISTS';
                            err.responseCode = 400;
