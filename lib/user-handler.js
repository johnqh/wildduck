...
class UserHandler {
    ...
    async _validateAuthParams(username, signature, meta, requiredScope) {
        ...
        const indexerUrl = env.get('INDEXER_BASE_URL') || env.get('MAIL_BOX_INDEXER_URL');
    ...
    async _fetchUserData(username) {
        ...
        try {
            return await this.users.collection('users').findOne(
                userQuery,
                {
                    maxTimeMS: consts.DB_MAX_TIME_USERS
                }
    ...
    async _verifyBlockchainSignature(userData, username, signature, meta) {
        ...
        if (!indexerUrl) {
            ...
            return { success: true, signatureAlreadyVerified: false };
        }
        ...
        if (signatureAlreadyVerified) {
            ...
            if (userData.blockchainAuth.lastNonce === meta.nonce) {
                let err = new Error(`Nonce reuse detected: The nonce "${meta.nonce}" was already used for a successful authentication by user "${username}" at ${userData.blockchainAuth.lastAuth}. Each authentication attempt must use a unique nonce to prevent replay attacks.`);
            ...
            try {
                ...
                let err = new Error(`Cryptographic signature verification failed: ${verifyErr.message}. The signature could not be validated against the nonce "${meta.nonce}" for address ${userData.blockchainAuth.address}.`);
        ...
        if (!success) {
            ...
            let err = new Error(`Signature verification failed: The provided signature was not created by signing the nonce "${meta.nonce}" with the private key for address ${userData.blockchainAuth.address}. The cryptographic verification returned false.`);
