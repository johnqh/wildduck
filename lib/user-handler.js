...
class UserHandler {
    ...
    async _handleAuthSuccess(userData, meta, requiredScope, username, signatureAlreadyVerified) {
        ...
        // Check if user has mailboxes, create defaults if none exist
        await this._ensureDefaultMailboxes(userData);
    ...
    /**
     * Ensures user has default mailboxes created
     * @private
     */
    async _ensureDefaultMailboxes(userData) {
        try {
            const existingMailboxes = await this.database.collection('mailboxes').countDocuments(
                { user: userData._id },
                { maxTimeMS: consts.DB_MAX_TIME_MAILBOXES }
            );

            if (existingMailboxes === 0) {
                log.info('USER', 'Creating default mailboxes for user %s (no existing mailboxes found)', userData._id);
                
                // Create default mailboxes for this user
                let mailboxes = this.getMailboxes(userData.language || 'en').map((mailbox) => {
                    mailbox.user = userData._id;
                ...
                if (mailboxes.length) {
                    ...
                    log.info('USER', 'Created %d default mailboxes for user %s', mailboxes.length, userData._id);
                }
            }
        } catch (mailboxErr) {
            log.error('DB', 'Failed to create default mailboxes for user %s: %s', userData._id, mailboxErr.message);
            // Don't fail authentication if mailbox creation fails
        }
    }
