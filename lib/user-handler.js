...
const { publish, USER_CREATED, USER_DELETE_STARTED, USER_DELETE_CANCELLED } = require('./events');

/**
 * UserHandler manages user accounts, authentication, and address resolution.
 * Supports both traditional password authentication and blockchain-based
 * signature authentication for Web3 identities.
 *
 * @class UserHandler
 */
class UserHandler {
    /**
     * Initialize UserHandler with database connections and dependencies
     *
     * @param {Object} options - Configuration options
     * @param {Object} options.database - Main database connection
     * @param {Object} [options.users] - Users database connection (defaults to database)
     * @param {Object} options.redis - Redis connection for caching and rate limiting
     * @param {Function} [options.loggelf] - GELF logging function
     * @param {Object} [options.messageHandler] - Message handler instance for counters
     */
    constructor(options) {
    ...
    /**
     * Resolve email address to user account data with support for aliases,
     * wildcards, and auto-creation of blockchain addresses.
     *
     * @param {string} address - Email address to resolve
     * @param {Object} [options] - Resolution options
     * @param {boolean} [options.wildcard] - Allow wildcard matching
     * @param {boolean} [options.create] - Auto-create account if not found
     * @param {Object|boolean} [options.projection] - MongoDB projection or false
    ...
    /**
     * Async version of resolveAddress with enhanced blockchain support.
     * Attempts exact match, domain aliases, wildcards, and auto-creation.
     *
     * @param {string} address - Email address to resolve
     * @param {Object} [options] - Resolution options
     * @returns {Promise<Object>} Address data with user and target information
     */
    async asyncResolveAddress(address, options) {
        options = options || {};
        let wildcard = !!options.wildcard;

        // Normalize the address by removing labels (plus addressing) and dots for Gmail-style aliases
        // Example: john.doe+label@example.com becomes johndoe@example.com for lookup
        address = tools.normalizeAddress(address, false, {
            removeLabel: true,
            removeDots: true
        });

        // Split address into local part (username) and domain
        let atPos = address.indexOf('@');
        let username = address.substring(0, atPos);
        let domain = address.substring(atPos + 1);

        // Build MongoDB projection for the fields we want to retrieve
        // Always include user and targets for basic functionality
        let projection = {
            user: true,
            targets: true
        };

        // Add any additional projection fields requested by caller
        Object.keys(options.projection || {}).forEach(key => {
            projection[key] = true;
        });

        // Allow disabling projection entirely if explicitly set to false
        ...
        try {
            ...
            // STEP 1: Try exact address match
            // Look for the exact normalized address in the addresses collection
            ...
            // STEP 2: Try domain alias resolution
            // Check if the domain has an alias mapping (e.g., alias.com -> main.com)
            let aliasDomain;
            ...
            if (aliasData) {
                ...
                // Try the same username with the resolved domain
                addressData = await this.users.collection('addresses').findOne(
                    {
            ...
            // STEP 3: If wildcards not allowed, stop here
            if (!wildcard) {
                return false;
            }

            // STEP 4: Wildcard and catch-all address resolution
            // This handles patterns like *@domain.com, user@*, *user@domain.com, etc.

            // Ensure addrview is included in projection for wildcard matching logic
            projection.addrview = true;

            // Generate all possible wildcard patterns for this address
            // Example: for john.doe@example.com, generates: *@example.com, john.*@example.com, etc.
            ...
            // Sort wildcard patterns by specificity (longer = more specific)
            // This ensures we prefer more specific matches over general catch-alls
            let sortedDomainPartials = partialWildcards.map(addr => addr.replace(/^\*/, '')).sort((a, b) => b.length - a.length);
            let sortedAliasPartials = [];

            // Also check wildcard patterns for the alias domain if one exists
            if (aliasDomain) {
            ...
            // Combine all partial patterns, with domain partials first (higher priority)
            let sortedPartials = sortedDomainPartials.concat(sortedAliasPartials);

            // Query for all potential wildcard matches
            ...
            if (addressMatches && addressMatches.length) {
                // Use WeakMap to associate each match with its specificity score
                let matchingPartials = new WeakMap();

                // Score each match based on how specific its pattern is
                addressMatches.forEach(addressData => {
                    let partialMatch = sortedPartials.find(partial => addressData.addrview.indexOf(partial) >= 0);
                    if (partialMatch) {
                        // Lower index = higher specificity = better match
                        matchingPartials.set(addressData, sortedPartials.indexOf(partialMatch));
                    }
                });

                // Sort matches by specificity score and return the most specific one
            ...
            // STEP 5: Global catch-all check
            // Final fallback: check for username-specific global catch-alls (e.g., "postmaster@*")
            addressData = await this.users.collection('addresses').findOne(
                {
                    addrview: username + '@*'
                },
            ...
            // Wrap database errors with appropriate response codes
        ...
        if (options.create) {
            ...
            if (!authDetails) {
                let err = new Error(
                    `Unable to resolve blockchain authentication address for "${username}". This could be due to network issues, invalid ENS/SNS name, or unsupported blockchain identifier.`
                );
    ...
    async rateLimit(tokenID, meta, count) {
        ...
        if (env.get('ENABLE_RATE_LIMITING') !== 'true') {
            return { success: true };
        }
     ...
     * @private
     */
    async _validateAuthParams(username, signature, meta, requiredScope) {
        // Check if indexer is configured
        const indexerUrl = env.get('INDEXER_BASE_URL');

        // If no indexer URL is set, bypass strict validation for testing
        if (!indexerUrl) {
            console.log(`Authentication validation bypassed for ${username} (INDEXER_BASE_URL not set)`);
            return true;
        }

        // Signature is required for blockchain authentication
        if (!signature) {
            await this._logAuthFailure(username, requiredScope, meta, 'Missing signature');
            return false;
        }

        // Nonce is required for signature verification
        if (!meta.nonce || !meta.message) {
            await this._logAuthFailure(username, requiredScope, meta, 'Missing nonce');
            return false;
        }
     ...
     * @private
     */
    async _logAuthFailure(username, requiredScope, meta, message, userId = null) {
        try {
            let authlogTime = userId;
            if (!authlogTime) {
                authlogTime = await this.users.collection('users').findOne(
                    {
                ...
                authlogTime = authlogTime && authlogTime._id;
            }

            meta.requiredScope = requiredScope;

            await this.logAuthEvent(authlogTime, {
     ...
     * @private
     */
    async _checkRateLimit(username, meta, isPreCheck = false) {
        const rateLimitRes = await this.rateLimit(username, meta, isPreCheck);
    ...
    async _fetchUserData(username) {
        ...
        try {
            return await this.users.collection('users').findOne(userQuery, {
                maxTimeMS: consts.DB_MAX_TIME_USERS
            });
        } catch (err) {
            err.responseCode = 500;
    ...
    /**
     * Validates blockchain identifier and prepares user data for creation
     * @private
     * @param {string} username - The blockchain username/address
     * @param {string} signature - The cryptographic signature
     * @param {object} meta - Authentication metadata including nonce
     * @returns {object|null} Validation result with actualUsername, address, and authDetails, or null if invalid
     */
    async _validateAndPrepareBlockchainUser(username, signature, meta) {
        // Check if indexer is configured - bypass user creation checks for testing
        const indexerUrl = env.get('INDEXER_BASE_URL');

        if (!indexerUrl) {
            console.log(`User auto-creation bypassed for ${username} (INDEXER_BASE_URL not set)`);
            // For bypass mode, create a mock success structure
            let actualUsername = username;
            let address = username + '@' + config.emailDomain;
            let authDetails = await getAuthenticationAddress(actualUsername);
        ...
        if (atPos !== -1) {
            actualUsername = address.substring(0, atPos);
            domain = address.substring(atPos + 1);
        } else {
        ...
        // Use nonce for cryptographic verification
        const signatureValid = await verifySignature(actualUsername, signature, meta.nonce);
        if (!signatureValid) {
            return null;
        }
    ...
    /**
     * Attempts to auto-create a user for valid blockchain identifiers
     * @private
     */
    async _attemptAutoCreateUser(username, signature, meta) {
        try {
            const validationResult = await this._validateAndPrepareBlockchainUser(username, signature, meta);
            if (!validationResult) {
                return null;
            }

            const { actualUsername, address, authDetails } = validationResult;

            // Create user with validated blockchain data
            ...
            const userData = await this.users.collection('users').findOne(
                { _id: newUserData },
                {
                    maxTimeMS: consts.DB_MAX_TIME_USERS
                }
    ...
    /**
     * Validates user account status (disabled, suspended, scope)
     * @private
     */
    async _validateUserStatus(userData, username, requiredScope, meta) {
        // Check if user account is disabled
        if (userData.disabled) {
            await this._checkRateLimit(userData._id, meta);
            await this._logAuthFailure(username, requiredScope, meta, 'User disabled', userData._id);
            return false;
        }
        ...
        if (userData.suspended) {
            await this._checkRateLimit(userData._id, meta);
            await this._logAuthFailure(username, requiredScope, meta, 'User suspended', userData._id);
            return false;
        }

        // Check if scope is disabled for user
    ...
    /**
     * Verifies blockchain signature for authentication
     * @private
     */
    async _verifyBlockchainSignature(userData, username, signature, meta) {
        // Check if indexer is configured - bypass signature verification for testing
        const indexerUrl = env.get('INDEXER_BASE_URL');
        if (!indexerUrl) {
            console.log(`Blockchain signature verification bypassed for ${username} (INDEXER_BASE_URL not set)`);
            return { success: true, signatureAlreadyVerified: true };
        }
        ...
        if (signatureAlreadyVerified) {
            ...
            if (userData.blockchainAuth.lastNonce === meta.nonce) {
                let err = new Error(
                    `Nonce reuse detected: The nonce "${meta.nonce}" was already used for a successful authentication by user "${username}" at ${userData.blockchainAuth.lastAuth}. Each authentication attempt must use a unique nonce to prevent replay attacks.`
                );
                ...
                err.lastAuthTime = userData.blockchainAuth.lastAuth;
                err.troubleshooting = {
                    issue: 'Nonce reuse detected - each authentication must use a unique nonce',
                    solution: 'Generate a new unique nonce for each authentication attempt',
                    security_note: 'Nonce reuse protection prevents replay attacks',
                    last_successful_auth: userData.blockchainAuth.lastAuth
                };
                throw err;
            }
            ...
            // Use nonce for cryptographic verification
            try {
                success = await verifySignature(username, signature, meta.message);
            } catch (verifyErr) {
                // Signature verification threw an error (malformed signature, etc.)
                let err = new Error(
                    `Cryptographic signature verification failed: ${verifyErr.message}. The signature could not be validated against the nonce "${meta.nonce}" for address ${userData.blockchainAuth.address}.`
                );
                ...
                err.signature_format = signature?.startsWith('0x') ? 'hex' : 'base64';
                err.blockchain_type = userData.blockchainAuth.type;
                err.troubleshooting = {
                    check_signature_format: 'Ensure signature is properly formatted (base64 for API calls, hex for IMAP)',
                    check_nonce: `Verify the signature was created by signing exactly: "${meta.nonce}"`,
                    check_address: `Ensure signature was created by private key for address: ${userData.blockchainAuth.address}`,
                    verify_blockchain_type: `Authentication type: ${userData.blockchainAuth.type}`
                };
                throw err;
            }
        }
        ...
        if (!success) {
            ...
            let err = new Error(
                `Signature verification failed: The provided signature was not created by signing the nonce "${meta.nonce}" with the private key for address ${userData.blockchainAuth.address}. The cryptographic verification returned false.`
            );
            ...
            err.blockchain_type = userData.blockchainAuth.type;
            err.troubleshooting = {
                check_signature_creation: `The signature must be created by signing exactly: "${meta.nonce}"`,
                check_private_key: `Ensure you're using the private key for address: ${userData.blockchainAuth.address}`,
                check_signature_format: 'API expects base64, IMAP expects hex format',
                verify_blockchain_type: `This account uses ${userData.blockchainAuth.type} authentication`,
                common_issues: [
                    'Wrong private key used for signature',
                    'Incorrect nonce signed',
                    'Signature format mismatch',
                    'Case sensitivity in address comparison'
                ]
            };
            throw err;
        }

        return { success, signatureAlreadyVerified };
    }

    /**
     * Updates user after successful authentication
     * @private
     */
    async _handleAuthSuccess(userData, meta, requiredScope, username, signatureAlreadyVerified) {
    ...
    async asyncAuthenticate(username, signature, requiredScope, meta) {
        ...
        const paramsValid = await this._validateAuthParams(username, signature, meta, requiredScope);
        if (!paramsValid) {
            return [false, false];
        }

        // Check rate limiting
        await this._checkRateLimit(username, meta, true);

        // Fetch user data from database
        let userData = await this._fetchUserData(username);

        if (!userData) {
            // Attempt to auto-create user for valid blockchain identifiers
            userData = await this._attemptAutoCreateUser(username, signature, meta);

            // If still no user data after auto-creation attempt
            if (!userData) {
                await this._checkRateLimit(username, meta);
                await this._logAuthFailure(username, requiredScope, meta, 'Unknown user');
                return [false, false];
            }
        }

        // Validate user account status
        const statusValid = await this._validateUserStatus(userData, username, requiredScope, meta);
        if (!statusValid) {
            return [false, false];
        }
        ...
        try {
            ...
            const { success, signatureAlreadyVerified } = await this._verifyBlockchainSignature(userData, username, signature, meta);
            console.log('Signature verification result:', { success, signatureAlreadyVerified });

            if (success) {
                console.log('=== CALLING _handleAuthSuccess ===');
                const authResponse = await this._handleAuthSuccess(userData, meta, requiredScope, username, signatureAlreadyVerified);
            ...
            await this._checkRateLimit(userData._id, meta);
            await this._logAuthFailure(username, requiredScope, meta, 'Invalid blockchain signature', userData._id);
    ...
    async create(data) {
        // Validate username format with indexer before proceeding
        const indexerUrl = env.get('INDEXER_BASE_URL');

        // Bypass validation if INDEXER_BASE_URL is not set
        if (!indexerUrl) {
            log.info('USER', 'Username validation bypassed for %s (INDEXER_BASE_URL not set)', data.username);
        } else {
            try {
                const validationResponse = await axios.get(`${indexerUrl}/api/addresses/validate/${encodeURIComponent(data.username)}`, {
                    timeout: 10000
                });

                if (!validationResponse.data.isValid) {
                    let err = new Error(`Invalid username format: ${validationResponse.data.error || 'Username format validation failed'}`);
                    err.responseCode = 400;
                    err.code = 'InvalidUsernameFormat';
                    throw err;
                }

                log.info('USER', 'Username validation passed for %s (type: %s)', data.username, validationResponse.data.addressType);
            } catch (validationErr) {
                if (validationErr.code === 'ECONNREFUSED' || validationErr.code === 'ENOTFOUND') {
                    log.error('USER', 'Indexer service unavailable at %s during username validation', indexerUrl);
                    let err = new Error('Username validation service unavailable. Please try again later.');
                    err.responseCode = 503;
                    err.code = 'ValidationServiceUnavailable';
                    throw err;
                } else if (validationErr.response && validationErr.response.data) {
                    let err = new Error(`Username validation failed: ${validationErr.response.data.error || 'Invalid username format'}`);
                    err.responseCode = 400;
                    err.code = 'InvalidUsernameFormat';
                    throw err;
                } else if (validationErr.responseCode) {
                    // Re-throw our custom errors
                    throw validationErr;
                } else {
                    log.error('USER', 'Username validation error: %s', validationErr.message);
                    let err = new Error('Username validation failed due to service error');
                    err.responseCode = 500;
                    err.code = 'ValidationServiceError';
                    throw err;
                }
            }
        }
        ...
        if (this.messageHandler && !data.emptyAddress) {
            try {
                // TODO: Implement updateUser method in message handler if needed
        ...
        if (data.featureFlags && Object.keys(data.featureFlags).length) {
            for (let featureFlag of Object.keys(data.featureFlags)) {
                if (data.featureFlags[featureFlag]) {
                    let res = await this.users.collection('users').updateOne({ _id: r.insertedId }, { $inc: { [`featureFlags.${featureFlag}`]: 1 } });
                    if (res) {
                        // nothing
                    }
                }
    ...
    async update(user, data) {
        ...
        if (data.featureFlags && Object.keys(data.featureFlags).length) {
            for (let featureFlag of Object.keys(data.featureFlags)) {
                try {
                    if (data.featureFlags[featureFlag]) {
                        let res = await this.users.collection('users').updateOne({ _id: user }, { $inc: { [`featureFlags.${featureFlag}`]: 1 } });
                        if (res) {
                            switch (featureFlag) {
        ...
        let defaultMailboxes = [
            {
                ...
                noInferiors: false,
                subscribed: true,
                name: defaults.inbox || (mailboxTranslations[language] && mailboxTranslations[language].INBOX) || 'Inbox'
            },
            {
                path: 'Sent Mail',
                specialUse: '\\Sent',
                ...
                noInferiors: false,
                subscribed: true,
                name: defaults.sent || (mailboxTranslations[language] && mailboxTranslations[language]['\\Sent']) || 'Sent Mail'
            },
            {
                path: 'Trash',
                specialUse: '\\Trash',
                ...
                noInferiors: false,
                subscribed: true,
                name: defaults.trash || (mailboxTranslations[language] && mailboxTranslations[language]['\\Trash']) || 'Trash'
            },
            {
                path: 'Drafts',
                specialUse: '\\Drafts',
                ...
                noInferiors: false,
                subscribed: true,
                name: defaults.drafts || (mailboxTranslations[language] && mailboxTranslations[language]['\\Drafts']) || 'Drafts'
            },
            {
                path: 'Junk',
                specialUse: '\\Junk',
                ...
                noInferiors: false,
                subscribed: false,
                name: defaults.junk || (mailboxTranslations[language] && mailboxTranslations[language]['\\Junk']) || 'Junk'
            }
        ];
            ...
            if (existingAccount.featureFlags && Object.keys(existingAccount.featureFlags).length) {
                ...
                for (let featureFlag of Object.keys(existingAccount.featureFlags)) {
                    if (existingAccount.featureFlags[featureFlag]) {
                        let res = await this.users.collection('users').updateOne({ _id: userQuery }, { $inc: { [`featureFlags.${featureFlag}`]: -1 } });
                        if (res) {
                            // nothing
                        }
                    }
    ...
    async checkAddress(username) {
        ...
        let domain = username.substring(username.indexOf('@') + 1);
