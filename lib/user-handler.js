...
class UserHandler {
    ...
    async asyncAuthenticate(username, password, requiredScope, meta, requirePassword = true) {
        meta = meta || {};
    ...
    async activateTemp(userId, data) {
        try {
            ...
            log.error('DB', 'Activate Fail user=%s error=%s', userId, error);
    ...
    async setAuthToken(user, accessToken) {
        ...
        let userData = await this.users.collection('users').findOne(
            ...
            { projection: { authVersion: true, username: true } }
        );
        let authVersion = Number(userData && userData.authVersion) || 0;

        // Determine role based on username
        let role = 'user';
        if (userData && config.api.rootUsername && userData.username === config.api.rootUsername) {
            role = 'root';
        }
