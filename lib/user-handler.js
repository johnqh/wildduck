    ...
    async rateLimitUser(tokenID, meta, count) {
        if (meta && meta.ip) {
            try {
                ...
                let res = await this.redis.multi().incr(key).expire(key, consts.USER_AUTH_WINDOW).exec();
                let isMember = res && res[0] && res[0][1];

                if (count) {
                    return { success: isMember <= consts.USER_AUTH_FAILURES };
                }

                let success = isMember <= consts.USER_AUTH_FAILURES;
