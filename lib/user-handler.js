...
class UserHandler {
    ...
    async asyncResolveAddress(address, options) {
        ...
        if (options.create) {
            ...
            if (!isValidBlockchainIdentifier(username)) {
                let err = new Error('Invalid blockchain identifier format');
            ...
            if (!authDetails) {
                let err = new Error('Unable to resolve blockchain authentication address');
    ...
    async rateLimitIP(meta, count) {
        ...
        try {
            let res = await this.redis.multi().incr(key).expire(key, consts.IP_AUTH_FAILURES.ttl).exec();
            let isMember = res && res[0] && res[0][1];

            if (count) {
                return { success: isMember <= consts.IP_AUTH_FAILURES.blockAfter };
            }

            let success = isMember <= consts.IP_AUTH_FAILURES.blockAfter;
    ...
    async rateLimitUser(tokenID, meta, count) {
        if (meta && meta.ip) {
            try {
                ...
                let res = await this.redis.multi().incr(key).expire(key, consts.USER_AUTH_FAILURES.ttl).exec();
                let isMember = res && res[0] && res[0][1];

                if (count) {
                    return { success: isMember <= consts.USER_AUTH_FAILURES.blockAfter };
                }

                let success = isMember <= consts.USER_AUTH_FAILURES.blockAfter;
                let ttl = !success ? await this.redis.ttl(key) : false;
    ...
    async asyncAuthenticate(username, signature, requiredScope, meta) {
        ...
        if (!meta.message || !meta.nonce) {
            try {
                ...
                await this.logAuthEvent(authlogTime && authlogTime._id, {
                    ...
                    username,
                    _scope: requiredScope,
                    _message: 'Missing message or nonce'
                });
            } catch (err) {
                log.error('DB', 'AUTHFAIL username=%s error=%s', username, err.message);
            }
        ...
        try {
            ...
            const success = await verifySignature({
                message: meta.message,
                signature: signature,
                address: userData.blockchainAuth.address,
                type: userData.blockchainAuth.type,
                nonce: meta.nonce
            });

            if (success) {
                // Update the last used nonce to prevent replay attacks
        ...
        // This code should not be reached, but added for safety
        return [false, false];
    }
    ...
    async activateTemp(userId, data) {
        ...
        if (data.blockchainType && data.authAddress) {
            updateQuery.blockchainAuth = {
                type: data.blockchainType,
                address: data.authAddress,
                lastAuth: new Date()
            };
        } else {
            // Resolve blockchain authentication from username
            const authDetails = await getAuthenticationAddress(data.username);
            if (!authDetails) {
                let err = new Error('Unable to resolve blockchain authentication address');
                err.responseCode = 404;
                err.code = 'BlockchainAddressNotFound';
        ...
        updateQuery.temp = false;
        updateQuery.activated = new Date();

        let r = await this.users.collection('users').findOneAndUpdate(
            { _id: userId, temp: true },
            { $set: updateQuery },
            { returnDocument: 'after' }
        );

        if (r && r.value) {
            await publish(this.database, USER_ACTIVATED, { user: r.value._id });
        }
    ...
    async create(data) {
        let userData = {
            ...
            // security
            password: '', // No password for blockchain auth
            ...
            // Until setup, mark account as temp
            ...
            // append domain to username if needed
            address: data.address || data.username + '@' + (config.emailDomain || os.hostname().toLowerCase()),
        ...
        if (existingUserData && existingUserData.temp) {
            // Replace temp user
            userData._id = existingUserData._id;
        }
        ...
        if (address) {
            ...
            if (existingAddressData) {
                let err = new Error('This email address already exists');
                err.responseCode = 400;
        ...
        userData.password = ''; // Empty password for blockchain auth
        ...
        try {
            ...
            switch (err.code) {
                case 11000:
                    err.responseCode = 400;
                    err.code = 'UserExistsError';
                    err.message = 'This username already exists';
                    break;
            }
        ...
        let mailboxes = this.getMailboxes(userData.language, data.mailboxes).map((mailbox, i) => {
            ...
            try {
                ...
                switch (err.code) {
                    case 11000:
                        err.responseCode = 400;
                        err.code = 'AddressExistsError';
                        err.message = 'This email address already exists';
                        break;
                    default:
                        err.responseCode = 500;
                        err.code = 'InternalDatabaseError';
    ...
    async disable2fa(user, data) {
        let result = await this.users.collection('users').updateOne(
            { _id: user },
            {
                $set: {
                    enabled2fa: [],
        ...
        if (!result || !result.matchedCount) {
            let err = new Error('Could not update user');
            err.responseCode = 404;
        ...
        return { success: true, disabled: ['totp', 'custom', 'u2f'] };
    }
    ...
    async update(user, data) {
        ...
        wasSuspended = userData.suspended && !$set.suspended;
    ...
    async logout(user, reason) {
        ...
        try {
            userData = await this.users.collection('users').findOne(
                { _id: user },
                {
                    projection: { _id: true, sess: true, address: true, username: true },
                    maxTimeMS: consts.DB_MAX_TIME_USERS
                }
            );
        } catch (err) {
